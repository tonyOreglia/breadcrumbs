...
func TestRetrieveNotes(t *testing.T) {
	...
	mock.ExpectQuery(`
		SELECT b.id, n.note, ST_X\(b.geog::geometry\), ST_Y\(geog::geometry\), ST_Z\(geog::geometry\), b.ts FROM breadcrumbs as b LEFT JOIN notes as n ON b.data_id = n.id WHERE ST_DWithin\(b.geog, ST_MakePoint\(100.000010000010, 200.000020000010\), \$1\)
...
func TestRetrieveAllNotes(t *testing.T) {
	...
	mock.ExpectQuery(`
		SELECT b.id, n.note, ST_X\(b.geog::geometry\), ST_Y\(geog::geometry\), ST_Z\(geog::geometry\), b.ts FROM breadcrumbs as b LEFT JOIN notes as n ON b.data_id = n.id WHERE n.note IS NOT NULL
		`).WillReturnRows(rows)
